...
module EXE (
  ...
  assign io_flush_req = ((io_br_br && (((! io_i_next_taken) || (io_i_next_pc != io_br_pc)) || fail)) || (io_i_csr_op != CsrOp_N));
  always @(posedge sys_clk or posedge sys_reset) begin
    if(sys_reset) begin
      io_o_real <= 1'b0;
      io_o_pc <= 32'h00000000;
...
module ID (
  ...
  assign when_ID_l899 = (instr_kind == Instr_EBREAK);
  assign when_ID_l901 = (instr_kind == Instr_ECALL);
  assign when_ID_l913 = (instr_kind == Instr_SRET);
  assign when_ID_l915 = (instr_kind == Instr_MRET);
  assign when_ID_l917 = (instr_kind == Instr_UNK);
  always @(posedge sys_clk or posedge sys_reset) begin
    if(sys_reset) begin
      io_o_real <= 1'b0;
      io_o_pc <= 32'h00000000;
